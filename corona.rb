...
def generate_badges(data, root)
  ...
  data[:series].keys.reverse.each do |key|
    ...
    url = URL.gsub('NAME', URI.encode(name).gsub(/[ -]/, '%20')).gsub('TOTAL', total).gsub('DELTA', delta).gsub('COLOUR', colour)
    puts "Writing #{path}"
